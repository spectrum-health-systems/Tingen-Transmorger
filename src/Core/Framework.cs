...
/// </summary>
/// <remarks>
/// The methods in this class validate configured directories and interact with
/// the user via message boxes when values are missing or paths do not exist.
/// In certain cases the user will be prompted to create missing directories or
/// the application will be terminated via <see cref="MainWindow.StopApp"/>.
/// </remarks>
class Framework
{
    /// <summary>
    /// Verifies the configuration by validating standard directories and, when in
    /// admin mode, administrator-only directories.
    /// </summary>
    ...
    /// <summary>
    /// Validates a set of directory mappings. For each entry this method ensures
    /// that a non-empty path exists and that the target directory exists on disk.
    /// </summary>
    /// <param name="directories">A dictionary mapping configuration keys to directory paths.</param>
    /// <remarks>
    /// This method will display message boxes for missing or invalid values and
    /// will prompt the user to create missing directories. Choosing not to create
    /// a required directory will terminate the application.
    /// </remarks>
    ...
    /// </summary>
    /// <param name="dir">A key/value pair where the key is the configuration name and the value is the path.</param>
    /// <remarks>
    /// If the user agrees the directory will be created. If the user declines the
    ...
    /// <summary>
    /// Shows an error message for a missing or empty configuration setting and
